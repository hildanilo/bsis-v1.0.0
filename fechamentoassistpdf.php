...
$pdf->Write(5,'Relatorio de pagamentos de Assistência');
$pdf->Ln(15);
/* Header Fim*/
/* Table */
...
$prod_check = $conexao->prepare ("SELECT DISTINCT `clientes`.`nome`,`clientes`.`sobrenome`, ".$_SESSION['fecht'].".`id_controle` ,".$_SESSION['fecht'].".`valor-total-controle` FROM ".$_SESSION['fecht']." join `clientes`  on   `id-cliente` = `id_cliente` where ".$_SESSION['fecht'].".id_fechamento = '".$_SESSION['controle']."'");
$prod_check->execute();
