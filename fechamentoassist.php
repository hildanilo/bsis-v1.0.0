...
<?php
	if(isset($_POST['enviarFrech'])){					
		...
		fechamentoGravarAssist($conn,$conexao,$lojal,$montadorr,$fech,$data_atual);
	}
?>
							...
							<?php
								switch($loja){
									...
									while($resp=mysqli_fetch_array($x)){
										$numero=$resp['id_assist'];
										$cliente=$resp['nome'];
										$valor=$resp['valor-total'];
										echo 
										"<tr>
											<td> $numero </td>
											<td> $cliente</td>
									...
									$x=mysqli_query($conn,"
									SELECT DISTINCT `clientes`.`nome`,`assistencia_sao_bernardo` .`id_assist`,`assistencia_sao_bernardo` .`valor-total` FROM `assistencia_sao_bernardo`  join `clientes` on `id_cliente` = `id-cliente`  WHERE `status` ='CONCLUIDO' AND `id_montador` ='$montador'");
									while($resp=mysqli_fetch_array($x)){
										$numero=$resp['id_assist'];
										$cliente=$resp['nome'];
										$valor=$resp['valor-total'];
